...
>>> sandbox = Sandbox.create(template_id="sha256:<boot-index-digest>")
>>> sandbox.sandbox_id
...
也可使用 `with Sandbox.create(template_id="sha256:<boot-index-digest>") as sandbox:` 上下文管理器，自动调用 `delete()`。
...
with Sandbox.create(template_id="sha256:<boot-index-digest>") as sbx:
...
**参数：**
- `template_id` (str, 可选): 要启动的 Boot Index digest；省略时使用 conchd 默认 Template
...
```python
# 从指定 Template 创建
sbx = Sandbox.create(template_id="sha256:<boot-index-digest>")
...
- Template ID 就是规范化的 Boot Index digest；可通过 `origin=checkpoint` 和 `boot_mode=resume` 识别其来源与启动能力
